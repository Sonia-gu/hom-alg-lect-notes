...
\end{to_suj}
\begin{proof}
$(\Rightarrow)$ $d = d\circ id_K = d\circ(ds + sd) = dsd$, т.е. K -- расщепим. Гомотопный нулю морфизм комплексов индуцирует нулевое отображение на когомологиях, а именно: $H(id_K) = id_{H(K)} = H(0) = 0$ $\Rightarrow H(K)=0$. $(\Leftarrow)$ Из расщепимости точных троек вида\begin{tikzcd}[sep=small] 0\arrow[r]& Z^{n} \arrow[r]& K^n \arrow[r]& B^{n+1} \arrow[r]& 0\end{tikzcd} следует, что $K^n = Z^{n} \oplus B^{n+1}$. Так как $Z^n\cong B^n$ положим $s^n(K^n) = B^n$ и получим гомотопию нулю $id_{K^\bullet}.
\end{proof}
\begin{to_suj}
...
\bee
\begin{tikzcd}[sep=small]
     \ldots\arrow[r]& \Hom_{\K{A}}(X, K^\bullet) \arrow[r]& Hom_{\K{A}}(X,L^\bullet) \arrow[r]&\Hom_{\K{A}}(X, M^\bullet) \arrow[r]&\Hom_{\K{A}}(X, K^\bullet[1])\ldots
\end{tikzcd}
    \eee
\end{to_lem}
