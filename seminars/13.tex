...
{\color{green}\mathbf{(3)}} \quad&Z_r^{pq}\bigcap d^{-1} Z_{r-1}^{p+r+1, q-r} = \left( d^{-1}\left(F^{p+r}K^{p+q+1}\right)\bigcap F^p K^{p+q} \right)\bigcap d^{-1}\left(Z_{r-1}^{p+r+1, q-r}\right)
  ...
  &&\subset Z_{r+1}^{pq}
\end{align*}
Таким образом, для любого элемента цикла нашёлся элемент в факторе, то есть \ref{eq:Z} и \ref{eq:B} также являются эпиморфизмом. Таким образом, заданные отображения действительно будут изоморфизмами.
\[
